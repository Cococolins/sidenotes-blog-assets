...
const dashboardJs = read("dist/dashboard/sidenotes-editor.js");
const dashboardCss = read("dist/dashboard/sidenotes-editor.css");
const dashboardSnippet = read(
  "dist/snippets/sidenotes-dashboard-footer.html",
);
const dashboardRevisionMatch = dashboardSnippet.match(
  /cdn\.jsdelivr\.net\/gh\/Cococolins\/sidenotes-blog-assets@([^/]+)\/dist\/dashboard\//,
);
const dashboardRevision = dashboardRevisionMatch?.[1] || "";
const dashboardRevisionIsFixed =
  dashboardRevision === `v${packageJson.version}` ||
  /^[0-9a-f]{40}$/.test(dashboardRevision);

const dashboardChecks = [
  [
    "Dashboard bundle enables the colin pilot",
    dashboardJs.includes("colin"),
  ],
  [
    "Dashboard bundle explicitly excludes sndaily",
    dashboardJs.includes("sndaily"),
  ],
  [
    "Dashboard bundle explicitly excludes ct",
    dashboardJs.includes('"ct"'),
  ],
  [
    "Dashboard bundle explicitly excludes hom",
    dashboardJs.includes("hom"),
  ],
  [
    "Dashboard bundle guards the Bear textarea",
    dashboardJs.includes("body_content"),
  ],
  [
    "Dashboard bundle includes the native fallback path",
    dashboardJs.includes("Bear 原生 Markdown 编辑器"),
  ],
  [
    "Dashboard bundle includes the pilot URL switch",
    dashboardJs.includes("sn-editor"),
  ],
  [
    "Dashboard CSS is scoped to its root",
    dashboardCss.includes("#sn-editor-root"),
  ],
  [
    "Dashboard CSS includes iOS safe-area spacing",
    dashboardCss.includes("env(safe-area-inset-bottom)"),
  ],
  [
    "Dashboard CSS force-hides the conflicting live toolbar",
    dashboardCss.includes(
      "#sn-md-toolbar[data-sn-editor-suspended=true]",
    ) &&
      dashboardCss.includes(
        ".markdown_line_fixer[data-sn-editor-suspended=true]{display:none!important}",
      ),
  ],
  [
    "Dashboard snippet uses a fixed tag or full commit SHA",
    dashboardRevisionIsFixed,
  ],
  [
    "Dashboard snippet does not use @latest",
    !dashboardSnippet.includes("@latest"),
  ],
  [
    "Dashboard snippet contains two SHA-384 integrity values",
    (dashboardSnippet.match(/integrity="sha384-[^"]+"/g) || []).length === 2,
  ],
];
...
for (const site of ["sidenotes", "daily", "tt"]) {
  const publicJs = read(`dist/${site}.js`);
  if (
    publicJs.includes("Sidenotes Dashboard Editor") ||
    publicJs.includes("@tiptap")
  ) {
    failed = true;
    console.error(`FAIL ${site} public bundle excludes Dashboard editor`);
  } else {
    console.log(`PASS ${site} public bundle excludes Dashboard editor`);
  }
}
...
for (const dependency of [
  ...
  if (packageJson.dependencies?.[dependency] !== "3.29.0") {
    failed = true;
    console.error(`FAIL ${dependency} is pinned to 3.29.0`);
  } else {
    console.log(`PASS ${dependency} is pinned to 3.29.0`);
  }
}
